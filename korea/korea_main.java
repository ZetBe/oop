...
public class korea_main {
	public static void main(String[] args) throws IOException{
	    PrintWriter pw = new PrintWriter("c:/korea.txt");//출력한 문자열들을 저장하기 위한 텍스트 파일 선언
		System.out.println("안녕하세요. 찾아 주셔서 감사합니다.");
		boolean a = true;
		List<Integer> list = new ArrayList<Integer>();//컬렉션 프레임워크 list 선언
		Scanner scanner = new Scanner(System.in);//문자열 및 정수를 입력하기 위한 scanner 선언
		int select = 0;
		while (a) {
			...
			System.out.println("6번: 종료"); // 1번부터 6번까지 종료하지 않으면 while문은 계속 실행된다.
			
			select = scanner.nextInt();
			System.out.print(select+"번을 선택하셨습니다.\n해당 번호로 이동합니다.\n");//해당 번호에 따라 해당 케이스로 이동한다.
				
				
			switch(select) {
			case 1:
				if (!list.contains(select)) { // list컬렉션에 입력한다. (모든 케이스에 있다.)
					list.add(select);
				}
				korea_case_1 k1 = new korea_case_1(); // 클래스 선언
				k1.explain();							//클래스 내부에 있는 함수 선언
				System.out.println("혹시 해당 설명을 파일에 저장하시겠습니까?");
				System.out.println("(Yes/No) 둘중 하나만 입력하시오.");
				String f = scanner.next();
				if (f.equals("Yes")) {					//출력한 문자들을 텍스트 파일에 저장한다.
					String data = k1.give_string_k1();
					pw.println(data);
					System.out.println("파일에 입력했습니다. 다음으로 넘어갑니다.");
				}
				else if (f.equals("No")) {				//그냥 넘어갈 수도 있다.
			...
			case 2:
				...
				korea_language kl = new korea_language();			//언어들을 출력하기 위한 클래스 선언
				...
				catch(NumberFormatException e) {				//예외처리(정수가 아니면 해당 함수 작동한다.)
					System.out.println("숫자가 아닙니다.");
				}
				
				if(lang==1) {
					kl.not_eng_kor();					//자식 클래스에 있는 함수 출력한다.
				}
				else if (lang==2) {
					kl.eng_kor();
				}
			...
			case 3:
				...
				if (o.equals("Yes")) {
					if (h==1) {
						String data = h1990.give();								//클래스 내부에 문자열을 저장해 두고 필요하면 리턴
						pw.println(data);
						System.out.println("파일에 입력했습니다. 다음으로 넘어갑니다.");
					}
					else if (h==2) {
						
						String data = h2000.give();
						pw.println(data);
						System.out.println("파일에 입력했습니다. 다음으로 넘어갑니다.");
					}
					else if (h==3) {
						String data = h2010.give();
						pw.println(data);
						System.out.println("파일에 입력했습니다. 다음으로 넘어갑니다.");
					}
				}
			...
			case 5:
				...
				if (num==1) {
					g = korea_case_5_gen.MUSIC;															//enum 열거를 사용해 필요한 곳에 사용
					now = g+"\n" + "방탄소년단: 2013년 6월 13일에 데뷔한 빅히트 뮤직 소속 대한민국 7인조 보이 그룹이다. "
					...
					now = g+"\n" + "오징어게임: 줄거리-빚에 쫓기는 수백 명의 사람들이 서바이벌 게임에 뛰어든다. 거액의 상금으로 새로운 삶을 시작하기 위해. "
					...
					now = g+"\n" + "농림축산식품부와 한식진흥원에 따르면 지난해 8~9월 뉴욕과 파리, 베이징 등 주요 도시 주민 8500명을 대상으로 설문조사를 한 결과, \n외국인들이 가장 선호하는 한식 메뉴는 ‘한국식 치킨’이 1위(16.1%)를 차지했다.\n"
							+ "출처: REAL FOODS\n"
					...
					now = g+"\n" + "배틀그라운드: 배틀그라운드는 김창한 대표의 게임 개발 의지와 도전정신에서 시작된 프로젝트로, 20여 명으로 구성된 작은 팀과 함께 배틀로얄이라는 새로운 게임 장르를 개척한 게임입니다."
					...
					now = g+"\n" + "기생충: 극과 극의 삶을 사는 두 가족의 만남이 빚어낸 신선한 스토리\r\n"
							+ "“같이 잘 살면 안 될까요?”\r\n"
			...
			case 6:
				boolean k = true;
				for(int i=1; i<=5; i++) {			
					if (!list.contains(i)) {		//리스트 컬렉션에 1번부터 5번까지 다 있으면 모든 항목을 다 봤다고 출력한다.
						k = false;
					}
				}
				if (k) {
				...
				if (exit.equals("Yes")){
					...
					BufferedReader reader = new BufferedReader(new FileReader("c:/korea.txt"));			//파일에 있는 문자들을 출력하려고 했지만 작동하지 않는다.
					String ch;
					while((ch = reader.readLine())!= null) {
						System.out.print(ch);
					}
					System.out.println("지금까지 봤던 글들은 모두 로컬 디스크 안에 텍스트파일로 기록하였습니다.");
					a = false;																			//여기서 while문은 끝나고 프로그램은 종료한다.
				}
